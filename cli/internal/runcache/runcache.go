...
func (tc TaskCache) RestoreOutputs(ctx context.Context, ecUI cli.Ui, targetLogger hclog.Logger, prettyPrefix string) (bool, error) {
	...
	case util.FullTaskOutput:
		targetLogger.Debug("log file", "path", tc.LogFileName)
		...
		if tc.LogFileName.FileExists() {
			...
			// this is the defaultLogReplayer
			tc.rc.logReplayer(targetLogger, replayUI, tc.LogFileName)
		}
	default:
